...
- Full analyzer suite is green: `1754/1754` tests in `PurelySharp.Test` targeting .NET 8 with the repo-pinned .NET SDK `9.0.315`.
...
- custom awaitable pattern members (`GetAwaiter`, `IsCompleted`, and `GetResult`) are now analyzed for source/boundary awaiters while preserving cataloged `Task`/`ValueTask` behavior
...
- nullable-display `System.IO.Path` parser overloads such as `GetExtension(string?)` are cataloged as deterministic string operations without relaxing the broader `System.IO` namespace
...
- integer `System.Buffers.Binary.BinaryPrimitives.Write*` overloads are now cataloged as impure when they write through `System.Span<byte>`
...
- `await foreach` over a source-defined async enumerable with an impure `GetAsyncEnumerator` now has direct regression coverage for existing diagnostics
...
- `out` arguments on otherwise pure calls now reject non-local targets such as fields while still allowing local/discard `out` targets for cataloged pure calls and dispatch-analyzed collection `TryGetValue` calls
...
- generic type-parameter comparison paths, including direct `Comparer<T>.Default.Compare`, now remain conservative instead of falling through to broad catalog purity when `Comparer<T>.Default` cannot be resolved
...
- `System.IO.FileNotFoundException.FileNotFoundException(string)` now uses the real namespace in the pure exception-constructor catalog, with direct coverage
...
- README installation guidance now matches the actual `PurelySharp` and `PurelySharp.Attributes` package IDs and local package workflow
  ...
  - provider-sensitive parsing coverage has been expanded across `DateOnly`, `TimeOnly`, `TimeSpan`, and `DateTimeOffset`, including `Parse`, `ParseExact`, `TryParse`, and `TryParseExact` provider/style overloads and the newer net8 span-based exact-parse overloads that previously slipped through the impure catalog; `TimeSpan.ParseExact(string, string, IFormatProvider?)` is now allowed only for constant standard formats with `CultureInfo.InvariantCulture`
  ...
  - legacy mutable wrapper APIs remain conservative for caller-owned mutable sources; `System.Array.AsReadOnly<T>(T[])` is impure for caller-owned arrays but allowed for tracked owned local arrays and `Array.Empty<T>()`
  ...
  - `System.DateTime.ParseExact`, `System.DateTime.TryParseExact`, and `System.DateTimeOffset.TryParseExact` stay conservative for environment-dependent exact parsing, with direct globalization regressions
  - `System.DateTimeOffset.ParseExact` remains conservative for multiple-format overloads and non-invariant/current-culture forms, while invariant single-roundtrip string/span overloads are allowed; the stale known-pure `System.DateTime.ParseExact(string, string, System.IFormatProvider)` catalog entry has been removed
