...
- Full analyzer suite is green: `1547/1547` tests in `PurelySharp.Test` targeting .NET 8 with the repo-pinned .NET SDK `9.0.315`.
...
- LINQ `GroupBy` and `ToLookup` default comparer paths now derive equality purity from the selected key type (`TKey`) instead of the source element type
