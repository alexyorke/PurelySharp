...
- concrete `SortedDictionary<TKey,TValue>.ContainsKey/TryGetValue`, `SortedList<TKey,TValue>.ContainsKey/TryGetValue`, `SortedSet<T>.Contains/TryGetValue`, `List<T>.BinarySearch(T)`, and `MemoryExtensions.BinarySearch<T>` calls now derive comparison purity from the key/element type instead of blindly trusting broad catalog entries
